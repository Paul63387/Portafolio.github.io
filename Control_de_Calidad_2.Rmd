...
abline(v = 770, col = "red")
abline(v = 790, col = "red")
```
...
Variacion_real <- 6*(3)
Cp <- Variacion_tolerada/Variacion_real;Cp
```

```{r}
Valor_indice_cp <- c("Cp ≥ 2", 
                     "Cp > 1.33",
                     "1 < Cp < 1.33",
                     "0.67 < Cp < 1",
                     "Cp < 0.67")
Clase_o_categoria <- c("Clase mundial",
                       "1",
                       "2",
                       "3",
                       "4")
Decision_si_el_proc_esta_centra <- c("Se tiene calidad Seis Sigma",
                                     "Adecuado",
                                     "Parcialmente Adecuado, requiere de un control estricto",
                                     "No adecuado para el trabajo, modificaciones serias",
                                     "No adecuado para el trabajo, modificaciones muy serias")
Tabla5_1 <- data.frame(Valor_indice_cp,
                       Clase_o_categoria,
                       Decision_si_el_proc_esta_centra)
Tabla5_1
...
Variacion_real <- 6*(3)
Variacion_tolerada <- ES - EI 
Cr <- Variacion_real/Variacion_tolerada;Cr
```
...
```{r}
a <- c("Cpm < 1", "Cpm > 1", "Cpm > 1.33")
b <- c("El proceso no cumple con especificaciones",
       "El proceso cumple con especificaciones",
       "El proceso cumple con especificaciones, pero además la media del proceso está dentro de la quinta parte central de la banda de especificación")
c <- data.frame(a,b);c
```

```{r}
Cpm <- (ES-EI)/(6*sqrt((3^2)+(mu-0.5*(ES+EI))^2));Cpm
```
...
```{r}
a <- c("Zm < 1.5", "Zm > 1.5", "Zm No se conoce")
b <- c("El proceso tiene un mejor control que el promedio",
       "El control es muy malo",
       "Se asume un valor de 1.5")
c <- data.frame(a,b);c
```
...
rbind(head(ejemplo1_med),tail(ejemplo1_med))
```
...
plot(ejemplo1_med$Media, type = "o", 
     ...
     main = "Gráfico X",
     col = "#EA80FC")
abline(h = ejemplo1_med$LCS, col = "white",lty = 2)
abline(h = ejemplo1_med$LC, col = "#00DAC6", lty = 2)
abline(h = ejemplo1_med$LCI, col = "white", lty = 2)
...
plot(ejemplo1_med$Rango, type = "o", 
     ...
     main = "Gráfico R",
     col = "#EA80FC")
abline(h = ejemplo1_med$LCS_R, col = "white",lty = 2)
abline(h = ejemplo1_med$LC_R, col = "#00DAC6", lty = 2)
abline(h = ejemplo1_med$LCI_R, col = "white", lty = 2)
...
rbind(head(ejemplo2, 5), tail(ejemplo2, 5))
```
...
plot(ejemplo2$Componentes.D, type = "o", 
     ...
     main = "Gráfico np",
     col = "#EA80FC")
abline(h = ejemplo2$LCS, col = "white",lty = 2)
abline(h = ejemplo2$LC, col = "#00DAC6", lty = 2)
abline(h = ejemplo2$LCI, col = "white", lty = 2)
```
